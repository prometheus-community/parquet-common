...
func TestParquetWriterParallel(t *testing.T) {
	bkt, err := filesystem.NewBucket(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { _ = bkt.Close() })
	...
	for _, testCase := range testCases {
		testName := fmt.Sprintf("pipeReaderWriter=%s", testCase.name)
		t.Run(testName, func(t *testing.T) {
			shardRowReaders, err := NewShardedTSDBRowReaders(ctx, h.MinTime(), h.MaxTime(), (time.Minute * 10).Milliseconds(), []Convertible{h}, &convertsOpts)
			require.NoError(t, err)
			defer func() {
				for _, rr := range shardRowReaders {
					_ = rr.Close()
				}
			}()

			errGroup := &errgroup.Group{}
			errGroup.SetLimit(convertsOpts.writeConcurrency)
			for shard, rr := range shardRowReaders {
				errGroup.Go(func() error {
					var err error
					labelsProjection, err := rr.Schema().LabelsProjection()
					if err != nil {
						return errors.Wrap(err, "get labels projection")
					}
					chunksProjection, err := rr.Schema().ChunksProjection()
					if err != nil {
						return errors.Wrap(err, "get chunks projection")
					}
					outSchemaProjections := []*schema.TSDBProjection{
						labelsProjection, chunksProjection,
					}
			...
			totalShards := len(shardRowReaders)
			rowsPerShard := (totalNumberOfSeries / totalShards) + 1
			remainingRows := totalNumberOfSeries
			chunksDecoder := schema.NewPrometheusParquetChunksDecoder(chunkenc.NewPool())
			buf := make([]parquet.Row, totalNumberOfSeries)
			fSeries := make([]labels.Labels, 0, totalNumberOfSeries)
			...
			for i := 0; i < totalShards; i++ {
				...
				if i < totalShards-1 {
					...
					require.Equal(t, remainingRows, n)
				}
