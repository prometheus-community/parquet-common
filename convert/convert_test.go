...
func Test_CreateParquetWithReducedTimestampSamples(t *testing.T) {
	...
	// Check metadatas
	for _, fileView := range []storage.ParquetFileView{shard.LabelsFile(), shard.ChunksFile()} {
		require.Equal(t, schema.MetadataToMap(fileView.Metadata().KeyValueMetadata)[schema.MinTMd], strconv.FormatInt(mint, 10))
		require.Equal(t, schema.MetadataToMap(fileView.Metadata().KeyValueMetadata)[schema.MaxTMd], strconv.FormatInt(maxt, 10))
		require.Equal(t, schema.MetadataToMap(fileView.Metadata().KeyValueMetadata)[schema.DataColSizeMd], strconv.FormatInt(datColDuration.Milliseconds(), 10))
	}
